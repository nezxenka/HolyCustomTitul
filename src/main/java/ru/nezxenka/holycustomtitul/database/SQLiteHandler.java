...
public class SQLiteHandler implements DatabaseHandler {
    ...
    public SQLiteHandler(HolyCustomTitul plugin) {
        this.plugin = plugin;
    }
    ...
    public Connection getConnection() {
        try {
            if (connection == null || connection.isClosed()) {
                String filename = plugin.getConfigManager().getConfig("database").getString("sqlite.filename", "titles.db");
    ...
    public void initializeTables() {
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement()) {
            ...
            stmt.execute("CREATE TABLE IF NOT EXISTS player_tokens (player VARCHAR(16) NOT NULL, tokens INT DEFAULT 0, PRIMARY KEY (player))");
        } catch (SQLException e) {
            plugin.getLogger().severe("Ошибка инициализации таблиц SQLite: " + e.getMessage());
        }
    }
    ...
    public void closeConnection() {
        try {
            ...
            plugin.getLogger().severe("Ошибка закрытия соединения SQLite: " + e.getMessage());
        }
    }
    ...
    public CompletableFuture<Integer> getPlayerTokens(String playerName) {
        return CompletableFuture.supplyAsync(() -> {
            try (Connection conn = getConnection();
                ...
                plugin.getLogger().warning("Ошибка получения токенов: " + e.getMessage());
            }
            return 0;
        });
    }
        ...
        return CompletableFuture.runAsync(() -> {
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(
                         "INSERT INTO player_tokens (player, tokens) VALUES (?, ?) ON CONFLICT(player) DO UPDATE SET tokens = tokens + ?")) {
                ...
                plugin.getLogger().warning("Ошибка добавления токенов: " + e.getMessage());
            }
        });
    }
    ...
    public CompletableFuture<Void> removePlayerTokens(String playerName, int amount) {
        return CompletableFuture.runAsync(() -> {
            try (Connection conn = getConnection();
                ...
                plugin.getLogger().warning("Ошибка удаления токенов: " + e.getMessage());
            }
        });
    }
    ...
    public CompletableFuture<Void> addPendingTitle(String playerName, String title) {
        return CompletableFuture.runAsync(() -> {
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(
                         "INSERT OR IGNORE INTO pending_titles (player_name, title) VALUES (?, ?)")) {

                ps.setString(1, playerName);
                ps.setString(2, title);
                ps.executeUpdate();
            } catch (SQLException e) {
                plugin.getLogger().warning("Ошибка добавления титула: " + e.getMessage());
            }
        });
    }
        ...
        return CompletableFuture.runAsync(() -> {
            try (Connection conn = getConnection()) {
                try (PreparedStatement ps = conn.prepareStatement(
                        "INSERT OR IGNORE INTO player_titles (player_name, title) VALUES (?, ?)")) {
                    ps.setString(1, playerName);
                    ps.setString(2, title);
                    ps.executeUpdate();
                }
                ...
                plugin.getLogger().warning("Ошибка одобрения титула: " + e.getMessage());
            }
        });
    }
    ...
    public CompletableFuture<Void> rejectTitle(String playerName, String title) {
        return CompletableFuture.runAsync(() -> {
            try (Connection conn = getConnection();
                ...
                plugin.getLogger().warning("Ошибка отклонения титула: " + e.getMessage());
            }
        });
    }
    ...
    public CompletableFuture<List<String[]>> getPendingTitles() {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection conn = getConnection();
                ...
                plugin.getLogger().warning("Ошибка получения pending titles: " + e.getMessage());
            }
            return titles;
        });
    }
    ...
    public CompletableFuture<List<String>> getPlayerPendingTitles(String playerName) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection conn = getConnection();
                ...
                plugin.getLogger().warning("Ошибка получения pending titles игрока: " + e.getMessage());
            }
            return titles;
        });
    }
    ...
    public CompletableFuture<Map<String, Timestamp>> getApprovedTitles(String playerName) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection conn = getConnection();
                ...
                plugin.getLogger().warning("Ошибка получения approved titles: " + e.getMessage());
            }
            return titles;
        });
    }
